...
int main() {
	...
	struct des_test {
		...
		int my_txt[8];
			...
			{	1	,{ 0x10, 0x31, 0x6E, 0x02, 0x8C, 0x8F, 0x3B, 0x4A },
					 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
			},
			{	2	,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
					 { 0x95, 0xF8, 0xA5, 0xE5, 0xDD, 0x31, 0xD9, 0x00 },
			},
			{ 3		,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
					 { 0xDD, 0x7F, 0x12, 0x1C, 0xA5, 0x01, 0x56, 0x19 },
			},
			{ 4		,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
					 { 0x2E, 0x86, 0x53, 0x10, 0x4F, 0x38, 0x34, 0xEA },
			},
			{ 5		,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
					 { 0x4B, 0xD3, 0x88, 0xFF, 0x6C, 0xD8, 0x1D, 0x4F },
			},
			{ 6		,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
					 { 0x20, 0xB9, 0xE7, 0x67, 0xB2, 0xFB, 0x14, 0x56 },
			},
			{ 7		,{ 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 },
	...
	for (int i = 0; i < 10; i++)
	{
		memcpy(my_key, cases[i].my_key, 8 * sizeof(int));
		memcpy(my_txt, cases[i].my_txt, 8 * sizeof(int));
		//将数组内容转为bitset格式
		...
		//将bitset转为int类型
		cout << "the ciphertext of num " << dec << i + 1 << " is :" << endl;
		cout << endl;
		...
		//将bitset转为int类型
		cout << "the plaintext of num " << dec << i + 1 << " is:" << endl;
		cout << endl;
	...
	for (int i = 0; i < 10; i++)
	{
		memcpy(my_key, cases_2[i].my_key, 8 * sizeof(int));
		memcpy(my_txt, cases_2[i].my_txt, 8 * sizeof(int));
		//将数组内容转为bitset格式
		key = in_to_bit(my_key);
		cipher_text = in_to_bit(my_txt);
