// AES雪崩效应分析.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int tag = 0;	//用于区分下面两个存放情况
bitset<128> data_change_1[11];//明文修改前每次加密步骤得到的结果
bitset<128> data_change_2[11];//明文修改后每次加密步骤得到的结果
bitset<128> data_change_3[11];//密文修改前每次解密步骤得到的结果
bitset<128> data_change_4[11];//密文修改后每次解密步骤得到的结果


int all_sum_1 = 0;//存放第一组数据比特修改总位数
int all_sum_2 = 0;//存放第二组数据比特修改总位数

int average_tag = 0;

double average_1 = 0;//存放第一组平均值
double average_2 = 0;//存放第二组平均值
...
void encrypt(const unsigned char *plain_text, const unsigned char *key, unsigned char *cipher_text)
{
	...
	if (tag == 1)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				bitset<8> temp(plain_array[j][k]);
				for (int l = 0; l < 8; l++)
				{
					data_change_1[0][num] = temp[l];
					cout << data_change_1[0][num];
					num += 1;
				}
				cout << " ";
			}
	...
	if (tag == 2)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_2[0][num] = temp[l];
					cout << data_change_2[0][num];
					num += 1;
				}
				cout << " ";
			}
		...
		if (tag == 1)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					...
					for (int l = 0; l < 8; l++)
					{
						data_change_1[i][num] = temp[l];
						cout << data_change_1[i][num];
						num += 1;
					}
					cout << " ";
				}
		...
		if (tag == 2)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					...
					for (int l = 0; l < 8; l++)
					{
						data_change_2[i][num] = temp[l];
						cout << data_change_2[i][num];
						num += 1;
					}
					cout << " ";
				}
	...
	if (tag == 1)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_1[10][num] = temp[l];
					cout << data_change_1[10][num];
					num += 1;
				}
				cout << " ";
			}
	...
	if (tag == 2)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_2[10][num] = temp[l];
					cout << data_change_2[10][num];
					num += 1;
				}
				cout << " ";
			}
...
void decrypt(const unsigned char *cipher_text, const unsigned char *key, unsigned char *plain_text)
{
	...
	if (tag == 3)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				bitset<8> temp(cipher_array[j][k]);
				for (int l = 0; l < 8; l++)
				{
					data_change_3[0][num] = temp[l];
					cout << data_change_3[0][num];
					num += 1;
				}
				cout << " ";
			}
	...
	if (tag == 4)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_4[0][num] = temp[l];
					cout << data_change_4[0][num];
					num += 1;
				}
				cout << " ";
			}
		...
		if (tag == 3)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					...
					for (int l = 0; l < 8; l++)
					{
						data_change_3[i][num] = temp[l];
						cout << data_change_3[i][num];
						num += 1;
					}
					cout << " ";
				}
		...
		if (tag == 4)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					...
					for (int l = 0; l < 8; l++)
					{
						data_change_4[i][num] = temp[l];
						cout << data_change_4[i][num];
						num += 1;
					}
					cout << " ";
				}
	...
	if (tag == 3)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_3[10][num] = temp[l];
					cout << data_change_3[10][num];
					num += 1;
				}
				cout << " ";
			}
	...
	if (tag == 4)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				...
				for (int l = 0; l < 8; l++)
				{
					data_change_4[10][num] = temp[l];
					cout << data_change_4[10][num];
					num += 1;
				}
				cout << " ";
			}
...
void data_compare(int round, bitset<128> data_1, bitset<128> data_2)
{
	int sum = 0;
	for (int i = 0; i < 64; i++)
	{
...
int main()
{
	...
	//修改明文后的测试样例
	unsigned char change_plain_text_1[] = { 0x00, 0x01, 0x00, 0x01, 0x01, 0xa1, 0x98, 0xaf, 0xda, 0x78, 0x17, 0x34, 0x3f, 0x15, 0x35, 0x66 };
	//unsigned char key[] = { 0x00, 0x01, 0x20, 0x01, 0x71, 0x01, 0x98, 0xae, 0xda, 0x79, 0x17, 0x14, 0x60, 0x15, 0x35, 0x94 };
	unsigned char change_cipher_text_1[16];
	unsigned char de_change_cipher_text_1[17];


	//AES加密
	tag = 2;//设置标记位
	encrypt(change_plain_text_1, key, change_cipher_text_1);

	//密文输出
	cout << "修改明文后得到的密文1为：" << endl;
	print_text(change_cipher_text_1, 16);
	...
	//考虑修改密文的情况
	//以第二组数据为例
	unsigned char plain_text_2[17];
	unsigned char key_2[] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
	unsigned char cipher_text_2[] = { 0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32 };
	//unsigned char en_plain_text_2[17];


	//AES解密
	tag = 3;//设置标记位
	decrypt(cipher_text_2, key_2, plain_text_2);
	...
	//修改密文后的测试样例
	unsigned char change_plain_text_2[17];
	//unsigned char key_2[] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
	unsigned char change_cipher_text_2[] = { 0x43, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32 };
	unsigned char en_change_plain_text_2[17];


	//AES解密
	tag = 4;//设置标记位
	decrypt(change_cipher_text_2, key_2, change_plain_text_2);

	//明文输出
	cout << "修改密文后的到的明文2为：" << endl;
	print_text(change_plain_text_2, 16);
	...
	for (int i = 0; i < 11; i++)
	{
		data_compare(i, data_change_3[i], data_change_4[i]);
	}


	average_2 = all_sum_2 * 1.0 / 11;
